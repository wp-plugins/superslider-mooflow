...
MooFlow.implement({
	...
	showTip: function(cur){
		this.tip.setStyles({'top':10,'display':'block'});
//		this.tipFx = new Fx.Morph(this.tip,{link:'cancel'}).start({'opacity': 0.8, 'top': this.icon.getCoordinates().top-this.tip.getSize().y+10});
		this.tipFx = new Fx.Morph(this.tip,{link:'cancel'}).start({'opacity': 0.6, 'top': 10});

	},
	
	hideTip: function(cur){
		this.tipFx.start({'opacity': 0, 'top': 10});
	},
